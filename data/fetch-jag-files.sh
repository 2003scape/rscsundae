...
if echo "a" | cksum -a sha256 -q 2>&1 >/dev/null; then
	CKSUM="cksum -a sha256 -q"
elif echo "a" | sha256 -q 2>&1 >/dev/null; then
	CKSUM="sha256 -q"
elif echo "a" | sha256sum 2>&1 >/dev/null; then
	CKSUM="sha256sum -b | cut -d' ' -f1"
elif echo "a" | gsha256sum 2>&1 >/dev/null; then
	CKSUM="gsha256sum -b | cut -d' ' -f1"
else
