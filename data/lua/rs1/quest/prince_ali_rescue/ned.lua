...
function ned_other_things(player, npc)
	...
	if resp == 1 then
		...
		npcsay(npc, "Be off wi' you, I have fought monsters that would turn your hair blue")
		npcsay(npc, "I don't need to be laughed at just 'cos I am getting a bit old")
	elseif resp == 2 then
		...
		if held(player, "ball of wool", 3) then
			...
			if resp == 1 then
				...
				mes(player, "Ned works with the wool. His hands move with a speed you couldn't imagine")
				delay(3)
				npcsay(npc, "Here you go, hows that for a quick effort? Not bad I think!")
	...
	elseif resp == 3 then
		...
		say(player, "Thanks Ned, maybe next time they will attack me face to face")
	end
end
